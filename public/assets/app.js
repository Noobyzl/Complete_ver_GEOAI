...
const radioColors = { G: "#00a6a6", L: "#f59e0b", U: "#7c3aed" };//u = #7c3aed, L = #f59e0b G = #00a6a6
const clusterColor = "#000000";
...
// Geographic extent of the baseline LST raster (assets/lst_baseline_source_raster.png).
// Leaflet order: [[south, west], [north, east]]. This is the raster's own footprint
// (~55 x 44 km), NOT the wider study/data extent in summary.bounds; using the latter
// stretches the image. Derived from the QGIS baseline layout (basemap landmark fit).
const BASELINE_RASTER_BOUNDS = [[2.9, 101.3], [3.3, 101.8]];
// Show only telecom towers that fall inside the baseline raster footprint.
// Set to false to show every tower in data/sites.json (whole study area).
const CLIP_SITES_TO_RASTER = true;
const presets = {
...
class TelecomCanvasLayer extends L.Layer {
  ...
  drawDot(context, item) {
    ...
    const dotColor = riskColors[item.risk] || "#30b9d9";
...
function initialiseMap() {
  ...
  baselineRasterLayer = L.imageOverlay("assets/lst_baseline_source_raster.png", BASELINE_RASTER_BOUNDS, {
...
function updateMetrics() {
  ...
  // summary.json describes the full portfolio; recompute from the visible sites when clipped.
  const baselineHigh = CLIP_SITES_TO_RASTER
    ? state.sites.filter((site) => riskForTemp(site[3]) === 2).length
    : state.summary.riskCounts.High;
  const baselineMean = CLIP_SITES_TO_RASTER
    ? state.sites.reduce((total, site) => total + site[3], 0) / state.sites.length
    : state.summary.baseline.mean;
  ...
  $("#metric-mean").textContent = `${baselineMean.toFixed(1)}°`;
...
async function initialise() {
  try {
    ...
    state.sites = CLIP_SITES_TO_RASTER
      ? siteData.rows.filter((site) => site[2] >= BASELINE_RASTER_BOUNDS[0][0] && site[2] <= BASELINE_RASTER_BOUNDS[1][0]
        && site[1] >= BASELINE_RASTER_BOUNDS[0][1] && site[1] <= BASELINE_RASTER_BOUNDS[1][1])
      : siteData.rows;
